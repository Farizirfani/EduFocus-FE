...
import { ChevronRight, BookOpen, PlayCircle, FileText, Pencil, BookmarkPlus, Share2, MoreHorizontal } from 'lucide-react';
import { getChaptersByCourse, getChapterById, getCourseById } from '@/services/api';
...
export default function CourseContentPage() {
      ...
      <div className="flex gap-6">
        <aside className="w-[220px] shrink-0 hidden xl:block">
          <div className="space-y-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-10 bg-dark-100 rounded-xl animate-pulse" />
            ))}
          </div>
        </aside>
        <div className="flex-1">
          <div className="bg-white rounded-2xl border border-dark-100 p-8 animate-pulse">
            <div className="h-6 bg-dark-100 rounded w-48 mb-4" />
            <div className="h-8 bg-dark-100 rounded w-64 mb-2" />
            <div className="h-4 bg-dark-50 rounded w-96 mb-8" />
            <div className="space-y-4">
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'Quiz':
        return <QuizApp chapterId={activeChapter?._id} />;
      case 'My Notes':
                ...
                <button
                  ...
                  className={`block w-full text-left px-3 py-2 rounded-lg text-[13px] transition-all duration-200 cursor-pointer ${
                    ch._id === activeChapter?._id
                      ...
                      : 'text-dark-500 hover:text-dark-700 hover:bg-dark-50 font-medium'
                  }`}
                >
                  {ch.title}
                </button>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 text-xs text-dark-400 mb-6">
          <span 
            ...
            className="hover:text-dark-600 cursor-pointer transition-colors"
          >
            Courses
          </span>
          <ChevronRight size={12} />
          <span className="hover:text-dark-600 cursor-pointer transition-colors">
            {course?.title ?? 'Course'}
          </span>
          <ChevronRight size={12} />
        ...
        <div className="bg-white rounded-2xl border border-dark-100 shadow-xs overflow-hidden">
          {/* Article Header */}
          <div className="px-8 pt-8 pb-6 border-b border-dark-100">
            <div className="flex items-center gap-3 mb-4">
              <span className="px-3 py-1 bg-primary-50 text-primary-700 text-xs font-semibold rounded-lg">
      ...
      <div className="w-12 shrink-0 hidden lg:block">
        <div className="sticky top-[92px] flex flex-col items-center gap-2">
            <button
              ...
              className={`w-10 h-10 rounded-xl bg-white border flex items-center justify-center transition-all duration-200 cursor-pointer shadow-xs ${
                course?.isBookmarked 
                    ? 'text-primary-600 border-primary-200' 
                    : 'text-dark-400 border-dark-100 hover:text-dark-600 hover:border-dark-200'
              }`}
            >
          ...
          {[Share2, MoreHorizontal].map((Icon, idx) => (
            <button
              key={idx}
              className="w-10 h-10 rounded-xl bg-white border border-dark-100 flex items-center justify-center 
                text-dark-400 hover:text-dark-600 hover:border-dark-200 transition-all duration-200 cursor-pointer shadow-xs"
            >
